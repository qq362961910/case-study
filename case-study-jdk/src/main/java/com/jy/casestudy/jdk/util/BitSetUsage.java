package com.jy.casestudy.jdk.util;
...
//        usageTest();
     ...
     * */
    public static void findOccurredNumber(String args[]) {
     ...
     * 持久化使用方法
     */
    private static void bitSetPersistTest() {
        BitSet bitSet = new BitSet();
        bitSet.set(1);
        bitSet.set(3);
        bitSet.set(5);
        ...
        System.out.println(String.format("====================== size: %d, length: %d, bytes: %s======================", bitSet.size(), bitSet.length(), Arrays.toString(data)));

        bitSet = BitSet.valueOf(data);
        for(int i=0; i<bitSet.length(); i++) {
            System.out.println(String.format("index: %d, value: %b", i, bitSet.get(i)));
        }
        data = bitSet.toByteArray();
        System.out.println(String.format("====================== size: %d, length: %d, bytes: %s======================", bitSet.size(), bitSet.length(), Arrays.toString(data)));
    }
